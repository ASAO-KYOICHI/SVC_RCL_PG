...
void COMCNT(void)
{
    ...
    while (RXLEN0 > 0) {
        ...
        if (SCRSTT == 0) {
            ...
            if ((RBUF0[(RXPTR0 + 5) & 0x0FF] == 0x50) && (RBUF0[(RXPTR0 + 6) & 0x0FF] == 0x43)) {
                ...
                if ((RBUF0[(RXPTR0 + 5) & 0x0FF] == 0x51) && (RBUF0[(RXPTR0 + 6) & 0x0FF] == 0x52)) {
                    /* コマンド正常"QR"である場合 */
                    ...
                    for (REG_00[1] = 9;REG_00[1] < 17;REG_00[1]++) {
                        /* 六回繰り返し */
                    ...
                    if (REG_00[1] <= 16) {
                        ...
                        /* 最後まで確認した後の場合(REG_00[1]が17) */
                        REG_00[1]--;/* 16にする */
                        
                    }
                    
                    FLMSTS = 19;/* コマンドのﾊﾞｲﾄ数セット */
                    SCRSTT = 0x01;/* QRコマンドセット */
    ...
    /* QRコマンド */
    REG_00[0] = RBUF0[(RXPTR0 + 11) & 0x0FF];/* 受信バッファ_アドレスの1000の位 */
    REG_00[1] = RBUF0[(RXPTR0 + 12) & 0x0FF];/* 受信バッファ_アドレスの100の位 */
    REG_00[2] = BNRCV(REG_00[0], REG_00[1]);/* ASCII -> Binary変換 */
    REG_01 = ((uint16_t)REG_00[2] * 100);/* ×100したものをセット */
    
    REG_00[0] = RBUF0[(RXPTR0 + 13) & 0x0FF];/* 受信バッファ_アドレスの10の位 */
    REG_00[1] = RBUF0[(RXPTR0 + 14) & 0x0FF];/* 受信バッファ_アドレスの1の位 */
    REG_00[2] = BNRCV(REG_00[0], REG_00[1]);/* ASCII -> Binary変換 */
    ...
    REG_01 = 0;/* クリア */
    REG_00[0] = RBUF0[(RXPTR0 + 15) & 0x0FF];/* 受信バッファ_読み込みデータ数の10の位 */
    REG_00[1] = RBUF0[(RXPTR0 + 16) & 0x0FF];/* 受信バッファ_読み込みデータ数の1の位 */
